...
class MissionInstance:
    ...
    def __init__(self, bot: BotAI, mp: MissionPrototype, k: bot_kite):
        self.bot = bot
        self.proto = mp
        self.kite = k
    ...
    async def testKite(self):
        bot = self.bot
        myForces = self.unitSelection.GetUnits(False).ready
        forces = myForces.filter(lambda unit: unit.can_attack)
        targetPosition = bot.enemy_start_locations[0].position
        for f in forces:
            await self.kite.Kite_normal(f, targetPosition)
