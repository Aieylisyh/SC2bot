...
class StalkerRushBot(BotAI):
  ...
  def DoIter5(self):
    ...
    if(pylonCount==0):
      ...
      if self.buildingsCenter == None:
        ...
        if(pylonCount>4):
          self.structures.ready(UnitTypeId.PYLON).random
        elif(pylonCount>1):
  ...
  async def on_step(self, iteration):
    ...
    if (
        pylonCount == 0
        or supplyUsed > 180 and self.supply_left < 6 and self.already_pending(UnitTypeId.PYLON) < 1
        ...
        or supplyUsed > 25 and self.supply_left < 5 and self.already_pending(UnitTypeId.PYLON) < 2
        ...
        if self.can_afford(UnitTypeId.PYLON):
            ...
            if pylonCount==0:
              ...
              await self.build(UnitTypeId.PYLON,  self.buildingsCenter.position + 5 * direction, 4)
            else:
    ...
    if self.structures(UnitTypeId.PYLON).ready and self.supply_workers + self.already_pending(UnitTypeId.PROBE) < self.townhalls.amount * 22 and nexus.is_idle:
        ...
        if self.supply_workers + self.already_pending(UnitTypeId.PROBE) <= self.townhalls.amount * 12 + pylonCount * 8 and nexus.is_idle:
          if self.can_afford(UnitTypeId.PROBE):
            nexus.train(UnitTypeId.PROBE)
    ...
    if self.structures(UnitTypeId.PYLON).ready:
        ...
        if self.structures(UnitTypeId.GATEWAY).ready:
            ...
            if not self.structures(UnitTypeId.CYBERNETICSCORE):
              ...
              if self.structures.ready(UnitTypeId.CYBERNETICSCORE) and self.can_afford(UnitTypeId.FORGE) and self.structures(UnitTypeId.FORGE).amount == 0 and self.already_pending(UnitTypeId.FORGE) == 0:
    ...
    if self.structures(UnitTypeId.PYLON).ready and self.structures(UnitTypeId.CYBERNETICSCORE).ready:
        pylon = self.structures(UnitTypeId.PYLON).ready.random
        gatewayCount = self.structures(UnitTypeId.GATEWAY).ready.amount + self.already_pending(UnitTypeId.GATEWAY)
        if(gatewayCount==4):
          if self.can_afford(UnitTypeId.ROBOTICSBAY) and self.structures(UnitTypeId.ROBOTICSBAY).amount==0 and self.already_pending(UnitTypeId.ROBOTICSBAY) == 0:
                await self.build(UnitTypeId.ROBOTICSBAY, near=pylon)

        if (self.townhalls.ready.amount + self.already_pending(UnitTypeId.NEXUS) >= self.target_base_count):
          if (gatewayCount < self.target_bg_count):
            if self.can_afford(UnitTypeId.GATEWAY):
                print("build gate way 1 "+str(gatewayCount))
                await self.build(UnitTypeId.GATEWAY, near=pylon)
        else:
          if (gatewayCount < 2):
            if self.can_afford(UnitTypeId.GATEWAY):
                print("build gate way 2 "+str(gatewayCount))
                await self.build(UnitTypeId.GATEWAY, near=pylon)
